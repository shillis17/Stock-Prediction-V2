...
def build_plots():
    ...
    for i in helper:
        findex = np.arange(1, i[3]+1)
        pindex = np.arange(-i[3]+1,1)
        lstmp = i[0].predictions[-i[3]:].values
        propp = i[1].forecast.yhat[-i[3]:].values
        hist = i[1].data.close[-i[3]:]
        fig, ax = plt.subplots(figsize=(12, 8))
        plt.autoscale()
        plt.tight_layout(pad=3)
        plt.plot(pindex,hist, 'b')
        plt.plot(findex,lstmp, 'r')
        plt.plot(findex,propp, 'k')
        plt.legend(['History','LSTM','Prophet'])
        plt.xlabel('Days Out (From Today)')
        plt.ylabel('Value (US$)')
        plt.title(i[2] + ' Predictions ' + str(i[3]) +' days')
        plt.savefig('../img/S3/' + i[2] + '_' + str(i[3]) + '.png')
        plt.close()
    ...
    for i in stocks:
        ...
        plt.xlabel('Date')
        plt.ylabel('Value (US$)')
        plt.savefig('../img/S3/' + i[0] + '_historical.png')
        plt.close()